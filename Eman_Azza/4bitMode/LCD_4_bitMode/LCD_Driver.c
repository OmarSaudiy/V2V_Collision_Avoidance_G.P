/*
* Auther : Azza Saeed
* About  : Interfacing Lcd Device Driver For RPI3
*/
...
/******************************** DEVICE DRIVER CREATION FUNCTIONS **************/


/* Meta Information */
MODULE_LICENSE("GPL");
MODULE_AUTHOR(" Azza Saeed ");
...
static int __init ModuleInit(void) {
	...
	for(i=0; i<10; i++) {
		if(gpio_set_direction(gpios[i], 0)) {
	...
	/* Init the display */
	lcd_command(Eight_bitMode);	/* Set the display for 8 bit data interface */

	lcd_command(DisplayOn_CursorOn);	/* Turn display on, turn cursor on, set cursor blinking */
...
static void __exit ModuleExit(void) {
	int i;
	lcd_command(Clear_Screen);	/* Clear the display */
	for(i=0; i<10; i++){
