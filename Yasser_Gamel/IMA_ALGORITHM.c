/*****************************************************************/
/* AUTHER   : MOHAMED YASSER & MOHAMED JAMIL                     */
/* VERSION  : V01                                                */
/* DATE     : 24/3/2023                                          */                           
/*****************************************************************/
...
int Ima_Algorithm(int dir_sec_car, int bridge_case_sec_car ) // var bridge_case_sec_car must be created in main_algo
{
    ...
    int bridge_case;
    dir_my_car = get_my_direction();          // get_my_direction() the mpu6050 >> should be created in mpu_driver
    bridge_case_my_car = get_bridge_case();   // get_bridge_case() from the mpu6050 >> should be created in mpu_driver
